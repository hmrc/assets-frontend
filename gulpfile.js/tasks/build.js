...
gulp.task('build:v3', ['clean:v3'].concat(globalTasks), (done) => {
  runSequence(
    ...
    'test:v3',
    ['error-pages:v3', 'concat:audit:v3', 'concat:encryption:v3', 'modernizr:v3', 'browserify:v3'],
    done
  )
})
...
gulp.task('build:v4', ['clean:v4'].concat(globalTasks), (done) => {
  runSequence(
    ...
    'test:v4',
    ['error-pages:v4', 'concat:audit:v4', 'concat:encryption:v4', 'modernizr:v4', 'browserify:v4'],
    done
  )
})
